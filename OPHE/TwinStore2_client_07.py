...
def client_run_enc(uid_prime, mid_prime, pw_prime, source_file_path, iv,run_time):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        a_2_prime_mid = inv(r_0_prime) * a_1_prime
        ...
        communication_scale_enc += com_bytes
        print("[Client] Sent ack payload:", ack)
        if ack == '0':
            print("认证失败")
            return
        elif ack == '1':
        ...
        communication_scale_enc += com_bytes
        b2 = b1 + pk * (-r1 % order)
        print("[Client] b0:", uid_prime +sid + mid_prime)
        print("[Client] ek:", point_to_str(ek))
        print("[Client] b2:", point_to_str(b2))
        t = H3(uid_prime +sid + mid_prime, point_to_str(ek), b2)
        print("ENC_t:", t)
...
def client_run_dec(uid_prime,mid_prime, pw_prime, iv,run_time):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        y_prime_mid = H3(uid_prime+sid, pw_prime, a_2m)
        end_time1 = time.time()
        ...
        b1_prime = b0_prime * bytes_to_int(y)
