...
def client_run_register(id, pw,run_time):
    HOST = '10.102.104.8'
    PORT = 25555
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        print(f"Register enter socket.connect!")
        ...
        a_1,com_bytes =recv_with_length(s)
        communication_scale_register += com_bytes
...
def client_run_enc(id_prime, mid_prime, pw_prime, source_file_path, iv,run_time):
    #########数据加密和对秘钥的封装###################################################################################################
    HOST = '10.102.104.8'
    PORT = 25555
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        payload = {'id_prime': id_prime, 'mid_prime': mid_prime, 'a_0_prime': a_0_prime,'b': b}
        com_bytes = send_with_length(s, payload)
        communication_scale_enc += com_bytes
        ...
        pk_gv = pk + v * g
        valid = nizk_verify(id_prime, mid_prime, g, pk_gv, a_1_prime_mid, a_0_prime, pi)  ############
        if valid:
        ...
        print("[Client] opakem_encaps_time:", run_time["opakem_encaps_time"])
        ...
        payload = {'ek': ek, 'id': id_prime, 'mid': mid_prime,'tag': tag}
        com_bytes = send_with_length(s, payload)
        communication_scale_enc += com_bytes
        ...
        r1 = generate_sk()
        b0 = k + g * r1
        com_bytes = send_with_length(s, b0)
        communication_scale_enc += com_bytes

        b1,com_bytes = recv_with_length(s)
        communication_scale_enc += com_bytes
        b2 = b1 + pk * (-r1 % order)
        t = H3(id_prime + mid_prime, point_to_str(ek), b2)
        print("b2:", point_to_str(b2))
        print("ENC_t:", t)
        com_bytes = send_with_length(s, t)
        communication_scale_enc += com_bytes
        #————————————————————————————————————————————————————灰色字迹—————————————————————————————————————————————————#
        ####################################加密和密钥封装完成！！！！###################################################
        end_time5 = time.time()
...
def client_run_dec(id_prime,mid_prime, pw_prime, iv,run_time):
    ####################################解密###################################################
    HOST = '10.102.104.8'
    PORT = 25555
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        payload = {'id_prime': id_prime, 'mid_prime': mid_prime, 'a_0': a_0}
        com_bytes = send_with_length(s, payload)
        communication_scale_dec += com_bytes
        ...
        a_1 = payload['a_1']
        a_1m = payload['a_1m']
        b0_prime = payload['b0_prime']


        a_2 = inv(r_0) * a_1
        a_2m = inv(r_0) * a_1m
        y = H3(id_prime, pw_prime, a_2)
        y_mid = H3(id_prime, pw_prime, a_2m)
        y_prime_mid = H3(id_prime, pw_prime, a_2m)
        end_time1 = time.time()
        ...
        com_bytes = send_with_length(s, b1_prime)
        communication_scale_dec += com_bytes
        ...
        run_time["total_decryption_time"] = (end_time5 - start_time1) * 1000
