...
def twinstore_encrypt(sk,g,uid_prime, mid_prime,pw_prime, m, Reg,Str,iv,sid_prime):
    ...
    # print("y:", y)
    t = H3(uid_prime +sid_prime+ mid_prime, point_to_str(ek), b2)
    print("b2:", point_to_str(b2))
    print("ENC_t:", t)
...
def twinstore_decrypt(c,tag,iv,sk,uid_prime, mid_prime, pw_prime,Str,sid_prime):
    ...
    k_prime_prime = Str[uid_prime]['ek'] * inv(b2_prime)
    ...
    print("ek:", point_to_str(Str[uid_prime]['ek']))
    t_prime = H3(uid_prime +sid_prime+ mid_prime, point_to_str(Str[uid_prime]['ek']), k_prime_prime * sk)
    if Str[uid_prime]['t'] == t_prime:
        print("OPAE解密成功")
    else:
        print("OPAE解密失败，t不匹配")
