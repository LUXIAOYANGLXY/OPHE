...
def server_run_enc(s,Reg,Str,run_time,sid):
        ...
        b1 = b0 * Reg[uid_prime]['sk']
        # b1 =  Reg[uid_prime]['sk'] * b0
...
def server_run_dec(s, Reg, Str,run_time,sid):
        ...
        if (uid_prime,mid_prime) not in Str:
            print("用户未注册，请检查")
            return None
        a_1 = inv((Reg[uid_prime]['sk']+ H2(str_to_bytes(uid_prime+sid))) % order) * a_0
        a_1m = inv((Reg[uid_prime]['sk'] + H2(str_to_bytes(uid_prime +sid+ mid_prime))) % order) * a_0
        ...
        start_time2 = time.time()
        payload = {'a_1': a_1, 'a_1m': a_1m, 'b0_prime': b0_prime,'sid':sid}
        send_with_length0(s, payload)
        ...
        k_prime_prime = Str[uid_prime,mid_prime]['ek'] * inv(b2_prime)
        print("[Server] type k_prime_prime:", k_prime_prime)
        print("[Server] k_prime_prime:", point_to_str(k_prime_prime))

        b__2 =k_prime_prime * Reg[uid_prime]['sk']
        print("[Server] uid_prime +sid + mid_prime:", uid_prime +sid + mid_prime)
        print("[Server] ek:", point_to_str(Str[uid_prime,mid_prime]['ek']))
        print("[Server] b__2:", point_to_str(b__2))
        t_prime = H3(uid_prime +sid + mid_prime, point_to_str(Str[uid_prime,mid_prime]['ek']), b__2)

        if Str[uid_prime,mid_prime]['t'] == t_prime:
            send_with_length0(s,'1')
            print("OPAE解密成功")
        else:
            send_with_length0(s,'0')
