...
class Client:
    internal_cipher_file_path = os.path.join(FILE_PATH, "internal/")  # 推荐使用 os.path.join()
    ...
    def connect_and_get_socket(self, address: str, port: int, name: str) -> socket.socket:
        if self.use_tls:
            ...
            # raw_sock = socket.create_connection((address, port))
            sock = context.wrap_socket(socket.socket(socket.AF_INET, socket.SOCK_STREAM))
            # sock = context.wrap_socket(raw_sock,name==name)
            ...
            # common_name = dict(cert['subject'])['commonName']
            # if common_name != name:
            #     raise Exception(f"Hostname verification failed: {common_name}")
    ...
    def ib_oprf(self, auth_server_address: str, auth_server_port: int, auth_server_name: str,
        ...
        try:
            ...
            # print(dir(self.curve))
            hash_point = self.curve.hash2curve(message)
            # hash_point =SimpleEcCurve.hash2curve(message)
            ...
            # 这里假设服务器返回的是盲化的点乘以私钥，客户端解盲后得到最终结果
            # 示例中省略了真正的服务端逻辑，此处只是示意
            # b_ec_point_derive_byte = blindedec_point_bytes  # 实际应从服务器接收
    ...
    def register(self, auth_server_address: str, auth_server_port: int, auth_server_name: str,
        ...
        try:
            ...
            # t = Utils.kdf(passphrase,sid, KDF1_SALT.encode(), MAC_KEY_LENGTH, KDF_HASH_REPETITIONS)
    ...
    def encrypt_ctr_big_file(self, source_path: str, dest_path: str, key: bytes) -> float:
        """
        Encrypt a file using AES-CTR mode and return encryption time.
        The IV is written at the beginning of the output file.
        """
        start_time = time.time()

        iv = os.urandom(16)
        # counter = Counter.new(128, prefix=iv[:16], initial_value=0)
        cipher = AES.new(key, AES.MODE_CTR, nonce=iv[:8])

        with open(source_path, 'rb') as fin, open(dest_path, 'wb') as fout:
            fout.write(iv)  # Write IV at the beginning
    ...
    # def secure_deposit_return_enc_time(self, bucket_name: str, key2: str, skey: bytes,
    #                                    source_file_path: str, internal_cipher_file_path: str) -> float:
    #     """
    #     Encrypt file and upload to S3, returns only encryption time.
    #     """
    #
    ...
    #
    #     try:
    #         # Enable Transfer Acceleration (if supported by your AWS SDK version)
    #         try:
    ...
    #         self.s3_client.upload_file(internal_cipher_file_path, bucket_name, key2)
    ...
    def secure_deposit(self, bucket_name: str, key4: str,key5:str, skey: bytes,
        ...
        """
        Same as secureDepositReturnEncTime but does not return timing.
        """
    ...
    def secure_retrieve(self, bucket_name: str, key2: str, skey: bytes, des_path: str,retry_max=3) -> None:
        """
        Retrieve encrypted file from S3, decrypt using CTR mode and save to disk.
        Does not return timing information.
        """
        ...
        for attempt in range(retry_max):
            try:
                ...
                try:
                    ...
                    # except Exception as e:
                    #     print(f"Failed to enable accelerate mode: {e}")
                    # Use TransferConfig for more robust download
            ...
            raise RuntimeError(f"Failed to download {key2} after {retry_max} retries.")
                    ...
                    # response = self.s3_client.get_object(Bucket=bucket_name, Key=key2)
                    # s3_stream = response['Body']
        try:
            # Read IV
            # iv = s3_stream.read(16)
            iv = buffer.read(16)
            # counter = Counter.new(128, prefix=iv[:16], initial_value=0)
        ...
        # except (ClientError, BotoCoreError) as e:
        #     print(f"S3 Error during secure retrieve: {e}")
        #     raise
        # except Exception as e:
    ...
    # def secure_retrieve_return_dec_time(self, bucket_name: str, key2: str, skey: bytes,
    #                                     des_path: str, internal_cipher_file_path: Optional[str] = None) -> float:
    #     """
    #     Retrieve encrypted file from S3, decrypt using CTR mode and save to disk.
    #     Returns the decryption time only (excluding download).
    #     """
    #     if os.path.exists(des_path):
    #         os.remove(des_path)
    #
    ...
    #         iv = s3_stream.read(16)
    #         counter = Counter.new(128, prefix=iv[:16], initial_value=0)
    #         cipher = AES.new(skey, AES.MODE_CTR, counter=counter)
    #
    #         # Measure decryption time
    #         start_time = time.time()
    #         with open(des_path, 'wb') as f_out:
    #             while True:
    #                 chunk = s3_stream.read(1024)
    #                 if not chunk:
    #                     break
    #                 decrypted_chunk = cipher.decrypt(chunk)
    #                 f_out.write(decrypted_chunk)
    ...
    #     """
    #     Encrypt file in parts and upload to S3 concurrently.
    ...
    def secure_deposit_optimization(self, bucket_name: str, key2: str, skey: bytes, source_path: str,
                                    internal_cipher_file_path: str) -> int:
        """
        Encrypt file in parts and upload to S3 after encryption completes.
        Returns number of parts.
        """
        file_size = os.path.getsize(source_path)
        part_number = max(1, round(math.sqrt(file_size / (1024 * 1024 * 20))))
    ...
    def upload_file_parts_to_s3(self, enc_list: List[int], part_number: int, cipher_path: str,
                                bucket_name: str, key2: str):
        """
        Upload encrypted file parts to S3.
        """
        print("【DEBUG】enter upload_file_parts_to_s3")
        try:
            # Enable Transfer Acceleration
    ...
    def secure_retrieve_optimization(self, part_number: int, bucket_name: str, key2: str,
        ...
        """
        Download encrypted parts concurrently and decrypt them in a single thread.
        """
        dec_streams = []
    ...
    def decrypt_ctr_big_file(self,source_path: str, dest_path: str, key: bytes):
        ...
        with open(source_path, 'rb') as fin, open(dest_path, 'wb') as fout:
            ...
            # counter = Counter.new(128, prefix=iv, initial_value=0)
            cipher = AES.new(key, AES.MODE_CTR, nonce=iv[:8])
