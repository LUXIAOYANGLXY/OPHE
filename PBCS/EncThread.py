...
#     def __init__(self, part_list: List[int], part_num: int, part_size: int, source_path: str, dest_path: str, key: bytes):
#         super().__init__()
...
#         for index in range(1, self.part_num):
#             with open(self.source_path, 'rb') as f:
#                 f.seek((index - 1) * self.part_size)
#                 part_bytes = f.read(self.part_size)
#
#             iv = get_random_bytes(16)
#             cipher = AES.new(self.key, AES.MODE_CTR, nonce=iv[:8])  # Use first 8 bytes as nonce
#             ct = cipher.encrypt(part_bytes)
#
#             out_file = os.path.join(self.dest_path, f'EncPart{index}')
#             with open(out_file, 'wb') as fout:
#                 fout.write(iv)  # Save full 16-byte IV
#                 fout.write(ct)
#
#             self.part_list.append(index)
#
#         # Last part
#         with open(self.source_path, 'rb') as f:
#             f.seek((self.part_num - 1) * self.part_size)
#             part_bytes = f.read(self.part_size)
#
#         iv = get_random_bytes(16)
...
#         ct = cipher.encrypt(part_bytes)
#
#         out_file = os.path.join(self.dest_path, f'EncPart{self.part_num}')
#         with open(out_file, 'wb') as fout:
#             fout.write(iv)
#             fout.write(ct)
#
#         self.part_list.append(self.part_num)
